...
export default function TasksTable({ tasks, queryParams = null, hideProjectColumn = false}) {
    ...
    const sortChanged = (name) => {
        // if the field is already sorted by that specific field,
        // then we just need to change the direction reverse
                    ...
                    <tbody>
                        {tasks.data.map((task) => (
                            <tr className="bg-white border-b dark:bg-gray-800 dark:border-gray-700" key={task.id}>
                                ...
                                <td className="px-3 py-2">
                                    <Link href={route('task.edit', task.id)} className="font-medium text-blue-600
                                                    dark:text-blue-500 hover:underline mx-1" >
                                        Edit
                                    </Link>
                                    <Link href={route('task.destroy', task.id)} className="font-medium text-red-600
